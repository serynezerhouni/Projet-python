...
def get_rebalance_dates(prices: pd.DataFrame, freq: str = "M") -> pd.DatetimeIndex:
    """
    Construit des dates de rebalancement (fin de mois par défaut) puis les mappe
    sur les derniers jours de bourse disponibles dans l'index des prix.
    """
    idx = prices.index
...
def performance_metrics(daily_returns: pd.Series) -> Dict[str, float]:
    """
    Stats classiques : CAGR, vol annualisée, Sharpe annualisé, max drawdown, total return.
    """
    ...
    sharpe = float((r.mean() * 252) / vol) if vol and vol > 1e-12 else np.nan
...
def backtest_pocheA_momentum(
    ...
    """
    Backtest long/short :
    - score calculé à chaque date de rebal
    - conversion score -> poids
    - rendements + coûts de transaction (bps * turnover)
    - sortie : daily_returns, weights_history, metrics
    """
    ...
    for i, reb_date in enumerate(rebal_dates):
        ...
        weights = compute_weights_from_scores(
            ...
            returns=returns.loc[:reb_date, prices.columns],
